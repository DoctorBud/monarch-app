...
var useSPA = env.getEnv().USE_SPA;
if (useSPA && useSPA === '1') {
    console.log('#USE_SPA enabled');
    useSPA = true;
}
else {
    useSPA = false;
}
var useStyles = env.getEnv().USE_STYLES;
if (useStyles && useStyles === '1') {
    console.log('#USE_STYLES enabled');
    useStyles = true;
}
else {
    useStyles = false;
...
// FIXME: This file should be read ONCE at launch, not on every request!!!
var assetMapJSON = fs.readFileSync('webpack-assets.json');
var assetMap = null;
if (assetMapJSON) {
    assetMap = JSON.parse(assetMapJSON);
}
else {
...
var     genExternalHrefObject = widgets.genExternalHrefObject;
...
var AsyncRequest = require('request-promise-native');
                                    // .defaults({
                                    //   forever: true
                                    // });
...
var pup_tent_dirs = [
                    ...
                    //obsolete 'node_modules/requirejs',
...
function renderWithBase(innerTemplate, info) {
  // console.log('renderWithBase', useSPA, innerTemplate, info);
  if (useSPA) {
    info.includes.navbar = '';
    var output = pup_tent.render(innerTemplate, info, 'spa_base.mustache');
  }
  else {
    var output = pup_tent.render(innerTemplate, info,'monarch_base.mustache');
  }
...
function buildEngine(defaultConfig, golrConfig) {
    ...
    fetch: function(tbl, key, val) {
        ...
        if (env.fs_existsSync(path)) {
            console.log('###CACHE HIT :', key, path);
            result = env.readJSON(path);
        }
        else {
            console.log('###CACHE MISS:', key, path);
        }
        return result;
    },
    store: function(tbl, key, val) {
        var path = "./cache/"+tbl+"/key-"+key+".json";
        console.log('###CACHE SAVE:', key, path);
        ...
        for (var i=0; i<files.length; i++) {
            ...
            if (file.indexOf("key-") > 0 &&
                ...
                if (match != null && file.indexOf(match) == -1) {
                    ...
                    console.log("###CACHE CLEAR: " + "./cache/" + file);
...
function addCoreRenderers(info, type, id, defaults) {
    ...
    info.monarch_use_styles = useStyles;
        ...
        for (var k in info.relationships) {
            ...
            if (propId == 'equivalentClass') {
                if (id == rel.subject.id){
                    ...
                    // console.error("Logic error: ", id, JSON.stringify(rel));
                }
            }
        }
    ...
    if (useSPA) {
      info.monarch_launchable.push('InitMonarchPage()');
      info.monarch_launchable.push('InitTabs()');
      info.monarch_launchable.push('InitFacetFilters()');
    }
    ...
    if (!info.isProduction) {
        ...
        var prodUrl = "https://monarchinitiative.org" + prodUrlSuffix;
...
function addPhenogridFiles(info) {
    ...
    info.pup_tent_js_variables.push({name:'global_phenogrid_conf_serverURL',
        value: engine.config.app_base});
    info.pup_tent_js_libraries.push("/phenogrid_config.js");
    info.monarch_launchable.push(
            '(window.configoptions = window.configoptions || {}).serverURL=global_phenogrid_conf_serverURL');
...
function errorResponse(errorStatus, errorMessage) {
    ...
    return renderWithBase(template, info);
}

function errorResponseWithObject(err) {
    console.log('####errorResponseWithObject:', err, Object.keys(err), err.status, err.message);
...
function getStatus(request) {
    ...
    status['platform'] = 'nodejs';
    status['platform_version'] = ('NodeJS: ' + process.version + ' HapiJS: ' + app.version);
    status['offerings'] = [
...
function defineRouteGET(path, paramNames, successHandler, errorHandler) {
  // console.log('defineRouteGET', useSPA, path, paramNames);
  if (useSPA) {
    web.wrapRouteGet(app, '/legacy' + path, paramNames, successHandler, errorHandler);
  }
  else {
    web.wrapRouteGet(app, path, paramNames, successHandler, errorHandler);
  }
}

function defineRoutePOST(path, paramNames, successHandler, errorHandler) {
  // console.log('defineRoutePOST', useSPA, path, paramNames);
  if (useSPA) {
    web.wrapRoutePost(app, '/legacy' + path, paramNames, successHandler, errorHandler);
  }
  else {
    web.wrapRoutePost(app, path, paramNames, successHandler, errorHandler);
  }
}

web.wrapRouteGet(app, '/status', [], statusHandler, errorResponse);

//Blank node handler
function blankNodeHandler(request) {
    var info = newInfo();
    addCoreRenderers(info);
    return renderWithBase('blank-node.mustache', info);
}

// Blank node route
web.wrapRouteGet(app, '/.well-known/genid/{id*}', ['id*'],blankNodeHandler, errorResponse);
...
function serviceStatusHandler(request) {
    ...
    return renderWithBase('status.mustache', info);
}

defineRouteGET('/services/status', [], serviceStatusHandler, errorResponse);
...
function pageByPageHandler(request, page) {
    ...
    if (useStyles) {
        pageTitles.styles = 'Stylesheet Test Page';
        pageLaunchables.styles = 'InitStyles()';
    }
    ...
    var launchable = pageLaunchables[page];
    if (launchable) {
        info.monarch_launchable.push(launchable);
    }

    info.title = pageTitles[page];
    if (info.title) {
        return renderWithBase(page + '.mustache', info);
    }
    else {
...
defineRouteGET('/page/{page}', ['page'], pageByPageHandler, errorResponse);

// block unwanted access
web.wrapRouteGet(app, '/robots.txt', [],
    function(request) {
        var info = newInfo();
        addCoreRenderers(info);
...
defineRouteGET('/docs/{p1}/{p2}/{p3}', ['p1', 'p2', 'p3'], docHandler, errorResponse);
defineRouteGET('/docs/{p1}/{p2}', ['p1', 'p2'], docHandler, errorResponse);
defineRouteGET('/docs/{p1}', ['p1'], docHandler, errorResponse);

web.wrapRouteGet(app, '/fonts/{f}', ['f'],
    function(request, f) {
        var path = '/fonts/'+f;
...
web.wrapRouteGet(app, '/image/{page}', ['page'],
...
web.wrapRouteGet(app, '/image/team/{page}', ['page'],
    function(request, page) {
...
web.wrapRouteGet(app, '/images/{file}.png', ['file'],
...
web.wrapRouteGet(app, '/node_modules/phenogrid/{filename*}', ['filename'],
...
web.wrapRouteGet(app, '/searchapi/{term}', ['term'], searchApiHandler, errorResponse);
async function searchApiHandler(request, term) {
    ...
    try {
        var results = await engine.search(term, request.query, ["category", "taxon_label"], rowsPerPage, start);
    }
    catch (e) {
        console.log('...searchApiHandler ERROR', e);
    }
    return formattedResults(results, 'json');
}
...
web.wrapRouteGet(app, '/suggestapi/{term}', ['term'], suggestApiHandler, errorResponse);
...
function searchHandler(request, term) {
    try {
        ...
        info.monarch_launchable.push('InitSearchResults()');

        info.title = 'Search Results: '+term;

        return renderWithBase('search_results.mustache', info);
    }
    catch(err) {
...
defineRouteGET('/search/{term}', ['term'], searchHandler, errorResponse);
...
function sourcesHandler(request, fmt) {
    try {
        ...
        info.sourcesTable = genTableOfDataSources(sources);
        ...
        return renderWithBase('sources.mustache', info);
    }
    catch(err) {
        return errorResponseWithObject(err);
    }
...
defineRouteGET('/about/sources.{fmt}', ['fmt'], sourcesHandler, errorResponse);
defineRouteGET('/about/sources', [], sourcesHandler, errorResponse);
...
if (useWebpack) {
    ...
    web.wrapRouteGet(app, '/dist/{file}.{type}', ['file', 'type'],
    ...
    web.wrapRouteGet(app, '/app.bundle.js', [],
        function(request) {
    ...
    web.wrapRouteGet(app, '/app.bundle.css', [],
        function(request) {
...
async function autocompleteByCategoryTermHandler(request,category,term,fmt) {
    // todo - we would like to normalize possible categories; e.g. phenotype --> Phenotype
    //var info = engine.searchSubstring(term, category);
    var info = await engine.autocomplete(term, category);
...
// Order matters here in the declaration of these routes.
web.wrapRouteGet(app, '/autocomplete/{category}/{term}.{fmt}', ['category', 'term', 'fmt'], autocompleteByCategoryTermHandler, errorResponse);
web.wrapRouteGet(app, '/autocomplete/{category}/{term}', ['category', 'term'], autocompleteByCategoryTermHandler, errorResponse);
...
// Order matters here in the declaration of these routes.
web.wrapRouteGet(app, '/autocomplete/{term}.{fmt}', ['term', 'fmt'], autocompleteByTermHandler, errorResponse);
web.wrapRouteGet(app, '/autocomplete/{term}', ['term'], autocompleteByTermHandler, errorResponse);
...
function diseaseLandingHandler(request) {
    ...
    var diseaseDist = env.readJSON('./widgets/dove/stats/MONDO-cache.json');
    ...
    return renderWithBase('disease_main.mustache', info);
}

if (useSPA) {
    web.wrapRouteGet(app, '/legacy/disease', [], diseaseLandingHandler, errorResponse);
    web.wrapRouteGet(app, '/legacy/disease/', [], diseaseLandingHandler, errorResponse);
}
else {
    web.wrapRouteGet(app, '/disease', [], diseaseLandingHandler, errorResponse);
    web.wrapRouteGet(app, '/disease/', [], diseaseLandingHandler, errorResponse);
}
...
function phenotypeLandingHandler(request) {
    ...
    return renderWithBase('phenotype_main.mustache', info);
}

defineRouteGET('/phenotype', [], phenotypeLandingHandler, errorResponse);
defineRouteGET('/phenotype/', [], phenotypeLandingHandler, errorResponse);
...
function addNonEmptyAnchors(info, inhibit_filters_kludge) {
    ...
    if (info.orthoDiseaseNum && info.orthoDiseaseNum > 0) {
        info.includes.ortholog_disease_anchor = addOrthoDiseaseAnchor(info);
        info.includes.ortholog_disease_table = addOrthoDiseaseTable();
    }
    ...
    if (info.functionNum && info.functionNum > 0) {
        // Add gene interaction table
        info.includes.function_anchor = addFunctionAnchor(info);
        info.includes.function_table = addFunctionTable();
    }
    if (info.anatomyNum && info.anatomyNum > 0) {
        // Add gene interaction table
        info.includes.anatomy_anchor = addAnatomyAnchor(info);
        info.includes.anatomy_table = addAnatomyTable();
    }
}
...
function modelByIdHandler(request, id, fmt) {
    try {
        ...
        if (/_/.test(id) && !/\:/.test(id)){
            ...
            return renderRedirect(genURL('model',newID));
        }
        ...
        info.xrefs = function() {
            return buildXrefsString(id, info.database_cross_reference);
        };
        ...
        return renderWithBase('node.mustache', info);
    }
    catch(err) {
        return errorResponseWithObject(err);
    }
...
// Order matters here in the declaration of these routes.
defineRouteGET('/model/{id}.{fmt}', ['id', 'fmt'], modelByIdHandler, errorResponse);
defineRouteGET('/model/{id}', ['id'], modelByIdHandler, errorResponse);
...
function genotypeByIdHandler(request, id, fmt) {
    try {
        ...
        if (/_/.test(id) && !/\:/.test(id)){
            engine.log("ID contains underscore, replacing with : and redirecting");
            var newID = id.replace("_",":");
            return renderRedirect(genURL('genotype',newID));
        }
...
// Order matters here in the declaration of these routes.
defineRouteGET('/genotype/{id}.{fmt}', ['id', 'fmt'], genotypeByIdHandler, errorResponse);
defineRouteGET('/genotype/{id}', ['id'], genotypeByIdHandler, errorResponse);
...
defineRouteGET('/case/{id}.{fmt}', ['id', 'fmt'], caseByIdHandler, errorResponse);
defineRouteGET('/case/{id}', ['id'], caseByIdHandler, errorResponse);
...
function geneLandingHandler(request) {
  console.log('geneLandingHandler');
    var info = prepLandingPage();
    ...
    return renderWithBase('gene_main.mustache', info);
}

defineRouteGET('/gene', [], geneLandingHandler, errorResponse);
defineRouteGET('/gene/', [], geneLandingHandler, errorResponse);
...
function genotypeLandingHandler(request){
    ...
    return renderWithBase('genotype_main.mustache', info);
}
...
function modelLandingHandler(request){
    ...
    return renderWithBase('model_main.mustache', info);
}

defineRouteGET('/model', [], modelLandingHandler, errorResponse);
defineRouteGET('/model/', [], modelLandingHandler, errorResponse);
defineRouteGET('/genotype', [], genotypeLandingHandler, errorResponse);
defineRouteGET('/genotype/', [], genotypeLandingHandler, errorResponse);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/neighborhood/{id}/{depth}/{direction}/{relationship}.{fmt}', ['id', 'depth', 'direction', 'relationship', 'fmt'], neighborhoodByIdDepthDirectionRelationshipHandler, errorResponse);
defineRouteGET('/neighborhood/{id}/{depth}/{direction}/{relationship}', ['id', 'depth', 'direction', 'relationship'], neighborhoodByIdDepthDirectionRelationshipHandler, errorResponse);
...
async function compareHandler(request, x, y, fmt) {
    var xs = x.split("+");
    ...
    //pass the arrays
    var info = await engine.compareEntities(xs,ys);

    return web.wrapJSON(info);
}

// Order matters here in the declaration of these routes.
defineRouteGET('/compare/{x}/{y}.{fmt}', ['x', 'y', 'fmt'], compareHandler, errorResponse);
defineRouteGET('/compare/{x}/{y}', ['x', 'y'], compareHandler, errorResponse);
...
function referenceByIdHandler(request, id, fmt) {
    ...
    //return renderRedirect(engine.expandIdToURL(id));
    var url = makeExternalURL(id);
    return renderRedirect(url);
}

// Order matters here in the declaration of these routes.
defineRouteGET('/reference/{id}.{fmt}', ['id', 'fmt'], referenceByIdHandler, errorResponse);
defineRouteGET('/reference/{id}', ['id'], referenceByIdHandler, errorResponse);


//Get orthologs/paralogs
async function queryOrthologsByIdHandler(request, id, fmt) {
    var res;
    var idList = id.split("+");
    if (idList == '.json'){
        res = errorResponse(500, "No gene IDs entered");
    } else {
        var info = await engine.fetchOrthologList(idList);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/query/orthologs/{id}.{fmt}', ['id', 'fmt'], queryOrthologsByIdHandler, errorResponse);
defineRouteGET('/query/orthologs/{id}', ['id'], queryOrthologsByIdHandler, errorResponse);
...
function literatureLandingHandler(request){
    ...
    return renderWithBase('literature_main.mustache', info);
}

defineRouteGET('/literature', [], literatureLandingHandler, errorResponse);
defineRouteGET('/literature/', [], literatureLandingHandler, errorResponse);
...
function literatureByIdHandler(request, id, fmt) {
    ...
    return renderWithBase('literature.mustache', info);
}

// Order matters here in the declaration of these routes.
defineRouteGET('/literature/{id}.{fmt}', ['id', 'fmt'], literatureByIdHandler, errorResponse);
defineRouteGET('/literature/{id}', ['id'], literatureByIdHandler, errorResponse);
...
async function scoreHandler(request) {
    ...
    return web.wrapJSON(await engine.getInformationProfile(annotation_profile,categories));
}

defineRouteGET('/score', [], scoreHandler, errorResponse);
defineRoutePOST('/score', [], scoreHandler, errorResponse);
...
web.wrapRoutePost(app, '/feedback', [], feedbackHandler, errorResponse);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/simsearch/disease/{id}.{fmt}', ['id', 'fmt'], simsearchDiseaseByIdHandler, errorResponse);
defineRouteGET('/simsearch/disease/{id}', ['id'], simsearchDiseaseByIdHandler, errorResponse);


async function simsearchPhenotypeByTargetTypeLimitItemsHandler(request) {
    ...
    var target = null;
    var info = {results:[]};
    info.results = await engine.searchByPhenotypeProfile(input_items,target_species,null,limit);

    return web.wrapJSON(info.results);
}

web.wrapRouteGet(app, '/simsearch/phenotype', [], simsearchPhenotypeByTargetTypeLimitItemsHandler, errorResponse);
web.wrapRoutePost(app, '/simsearch/phenotype', [], simsearchPhenotypeByTargetTypeLimitItemsHandler, errorResponse);
web.wrapRouteGet(app, '/simsearch/phenotype/', [], simsearchPhenotypeByTargetTypeLimitItemsHandler, errorResponse);
web.wrapRoutePost(app, '/simsearch/phenotype/', [], simsearchPhenotypeByTargetTypeLimitItemsHandler, errorResponse);
...
defineRouteGET('/phenopacket', [], phenoPacketHandler, errorResponse);


async function annotateTextHandler(request) {
    var q = request.query.q;
    var longestOnly = request.query.longestOnly;
    ...
    if (q == null || q == "") {
        ...
        info.results = await engine.annotateText(q, {longestOnly : longestOnly});
    ...
    return renderWithBase('annotate.mustache', info);
}

defineRouteGET('/annotate/text', [], annotateTextHandler, errorResponse);
...
function annotateMinimalTextHandler(request, fmt) {
    ...
    return renderWithBase('annotate_minimal.mustache', info);
}

// Order matters here in the declaration of these routes.
defineRouteGET('/annotate_minimal/text/{id}.{fmt}', ['id', 'fmt'], annotateMinimalTextHandler, errorResponse);
defineRouteGET('/annotate_minimal/text/{id}', ['id'], annotateMinimalTextHandler, errorResponse);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/scigraph/dynamic*/{id}.{fmt}', ['id', 'fmt'], scigraphDynamicHandler, errorResponse);
defineRouteGET('/scigraph/dynamic*/{id}', ['id'], scigraphDynamicHandler, errorResponse);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/admin/introspect.{fmt}', ['fmt'], adminIntrospectHandler, errorResponse);
defineRouteGET('/admin/introspect', [], adminIntrospectHandler, errorResponse);


defineRouteGET('/admin/cache/info', [],
...
defineRouteGET('/admin/clear-cache', [],
    function(request) {
...
async function _get_blog_data(label) {
  ...
  if ((now - _blog_data_last_time) < _blog_data_last_step && _blog_data_last_res.length > 0 ) {
    ...
    var rssContent = await AsyncRequest.get(rsrc);
...
function homeHandler(request) {
  ...
  return renderWithBase('home-page.mustache', info);
}

defineRouteGET('/', [], homeHandler, errorResponse);
...
async function phenotypeByIdHandler(request, id, fmt){

    //Curify ID if needed
    if (/_/.test(id) && !/\:/.test(id)){
        engine.log("ID contains underscore, replacing with : and redirecting");
        var newID = id.replace("_",":");
        return renderRedirect(genURL('phenotype',newID));
    }
    ...
    info.diseaseNum = await engine.fetchAssociationCount(disease_filter, 'subject');
    ...
    info.geneNum = await engine.fetchAssociationCount(gene_filter, 'subject');
    ...
    info.genotypeNum = await engine.fetchAssociationCount(genotype_filter, 'subject');
    ...
    info.modelNum = await engine.fetchAssociationCount(model_filter, 'subject');
    ...
    info.variantNum = await engine.fetchAssociationCount(variant_filter, 'subject');
...
defineRouteGET('/phenotype/{id}.{fmt}', ['id', 'fmt'], phenotypeByIdHandler, errorResponse);
defineRouteGET('/phenotype/{id}', ['id'], phenotypeByIdHandler, errorResponse);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/phenotype/{id}/{section}.{fmt}', ['id', 'section', 'fmt'], phenotypeByIdSectionHandler, errorResponse);
defineRouteGET('/phenotype/{id}/{section}', ['id', 'section'], phenotypeByIdSectionHandler, errorResponse);


function buildXrefsList(id, databaseCrossReference) {
    var xrefs = [];
    if (databaseCrossReference != null) {
        for (var xref_key in databaseCrossReference) {
            var xref = databaseCrossReference[xref_key];
            if (xref !== id) {
                xrefs.push(genExternalHrefObject('source', {id: xref}));
            }
        }
    }
    xrefs.unshift(genExternalHrefObject('source', {id: id}));
    return xrefs;
}

function buildXrefsString(id, databaseCrossReference) {
  var xrefs = [];
  if (databaseCrossReference != null) {
      for (var xref_key in databaseCrossReference) {
          var xref = databaseCrossReference[xref_key];
...
if (useSPA) {
  async function computeCounts(info, nodetype, id) {
    const anatomyFilters = {
      gene:     [
                  'object',
                  { field: 'subject_category', value: 'gene' },
                  { field: 'object_category', value: 'anatomical entity' },
                  { field: 'subject_closure', value: id}
                ],
    ...
    if (anatomyFilter) {
      info.anatomyNum = await engine.fetchAssociationCount(anatomyFilter.slice(1), anatomyFilter[0]);
    }
    ...
    if (diseaseFilter) {
      info.diseaseNum = await engine.fetchAssociationCount(diseaseFilter.slice(1), diseaseFilter[0]);
    }
    ...
    const functionFilters = {
      gene: [
                  'object',
                  { field: 'subject_category', value: 'gene' },
                  { field: 'object_category', value: 'biological process' },
                  { field: 'is_defined_by', value: 'https://data.monarchinitiative.org/ttl/go.ttl' },
                  { field: 'subject_closure', value: id }
               ],
    ...
    if (functionFilter) {
      info.functionNum = await engine.fetchAssociationCount(functionFilter.slice(1), functionFilter[0]);
    }
    ...
    if (geneFilter) {
      info.geneNum = await engine.fetchAssociationCount(geneFilter.slice(1), geneFilter[0]);
    }


    const genotypeFilters = {
      disease:  [
                  'subject',
    ...
    if (genotypeFilter) {
      info.genotypeNum = await engine.fetchAssociationCount(genotypeFilter.slice(1), genotypeFilter[0]);
    }


    const homologFilters = {
      gene: [
                  'object',
                  { field: 'object_category', value: 'gene' },
    ...
    if (homologFilter) {
      info.homologNum = await engine.fetchAssociationCount(homologFilter.slice(1), homologFilter[0]);
    }
    ...
    if (interactionFilter) {
      info.interactionNum = await engine.fetchAssociationCount(interactionFilter.slice(1), interactionFilter[0]);
    }
    ...
    if (literatureFilter) {
      info.literatureNum = await engine.fetchAssociationCount(literatureFilter.slice(1), literatureFilter[0]);
    }
    ...
    if (modelFilter) {
      info.modelNum = await engine.fetchAssociationCount(modelFilter.slice(1), modelFilter[0]);
    }


    const orthoPhenotypeFilters = {
      gene: [
                  'object',
                  { field: 'object_category', value: 'phenotype' },
    ...
    if (orthoPhenotypeFilter) {
      info.orthoPhenotypeNum = await engine.fetchAssociationCount(orthoPhenotypeFilter.slice(1), orthoPhenotypeFilter[0]);
      info.orthoPhenoNum = info.orthoPhenotypeNum;  // ick. Legacy inconsistent name
    }

    const orthoDiseaseFilters = {
      gene: [
                  'object',
                  { field: 'object_category', value: 'disease' },
                  { field: 'subject_ortholog_closure', value: info.cliqueLeader }
    ...
    if (orthoDiseaseFilter) {
      info.orthoDiseaseNum = await engine.fetchAssociationCount(orthoDiseaseFilter.slice(1), orthoDiseaseFilter[0]);
    }
    ...
    if (pathwayFilter) {
      info.pathwayNum = await engine.fetchAssociationCount(pathwayFilter.slice(1), pathwayFilter[0]);
    }
    ...
    if (phenotypeFilter) {
      info.phenotypeNum = await engine.fetchAssociationCount(phenotypeFilter.slice(1), phenotypeFilter[0]);
    }
    ...
    if (variantFilter) {
      info.variantNum = await engine.fetchAssociationCount(variantFilter.slice(1), variantFilter[0]);
    }
  }


  async function nodeByTypeIdHandler(request, nodetype, id) {
      // console.log('nodeByTypeIdHandler', nodetype, id);
      //Curify ID if needed
      if (/_/.test(id)){
          engine.log("ID contains underscore, replacing with : and redirecting");
          var newID = id.replace("_",":");
          return renderRedirect(genURL('disease',newID));
      }

      // Rendering.
      var info = newInfo(await engine.fetchClassInfo(id, {level:1}));

      await computeCounts(info, nodetype, id);

      // Duplicates code in addCoreRenderers()
      info.global_app_base = engine.config.app_base;
      info.global_scigraph_url = engine.config.scigraph_url;
      info.global_scigraph_data_url = engine.config.scigraph_data_url;
      info.global_solr_url = engine.config.golr_url;
      info.global_golr_conf = engine.config.golr_conf_json;

      // Compute inheritance

      const cliqueLeader = await engine.fetchCliqueLeader(id, engine.config.scigraph_data_url);
      const inheritanceList = await engine.fetchInheritance(cliqueLeader);
      info.cliqueLeader = cliqueLeader;
      // console.log('cliqueLeader', id, cliqueLeader);
      ...
      info.primary_xref = function() {return genExternalHref('source',{id : id})};
      info.xrefs = buildXrefsList(id, info.database_cross_reference);
      info.hasXrefs = checkExistence(info.database_cross_reference) || !!info.primary_xref;
...
async function diseaseByIdHandler(request, id, fmt) {
    ...
    const cliqueLeader = await engine.fetchCliqueLeader(id);

    // Rendering.
    const classInfo = await engine.fetchClassInfo(cliqueLeader, {level:1});
    const info = newInfo(classInfo);
    const inheritanceList = await engine.fetchInheritance(cliqueLeader);


    if (fmt != null && fmt !== 'jsonx') {
        return formattedResults(info, fmt, request);
    }
    ...
    info.phenotypeNum = await engine.fetchAssociationCount(phenotype_filter, 'object');
    ...
    info.pathwayNum = await engine.fetchAssociationCount(pathway_filter, 'object');
    ...
    info.literatureNum = await engine.fetchAssociationCount(literature_filter, 'subject');
    ...
    info.hasAka = function() {return checkExistence(info.synonyms)};
    info.hasEqual = function() {return checkExistence(info.equivalentClasses)};
    info.hasHeritability = function() {return checkExistence(info.inheritance)};
    info.hasModifier = function() {return checkExistence(info.modifier)};
    info.hasPhenotypes = info.phenotypeNum;
    ...
    info.pup_tent_js_variables.push({name:'globalID', value:id});
    info.pup_tent_js_variables.push({name:'globalLabel', value:info.label});
    info.pup_tent_js_variables.push({name:'root', value:'MONDO:0000001'});
    ...
    if (fmt === 'jsonx') { // For debugging purposes in Node.vue
        return web.wrapJSON(info);
    }
    else {
        return renderWithBase('node.mustache', info);
...
defineRouteGET('/disease/{id}.{fmt}', ['id', 'fmt'], diseaseByIdHandler, errorResponse);
defineRouteGET('/disease/{id}', ['id'], diseaseByIdHandler, errorResponse);
...
function anatomyByIdHandler(request, id, fmt) {
    ...
    if (/_/.test(id)) {
        engine.log("ID contains underscore, replacing with : and redirecting");
        var newID = id.replace("_",":");
        return renderRedirect(genURL('anatomy', newID));
    }
    ...
    info.equivalentClasses = info.equivalentClasses.filter(cls => !/.*neuinfo.*/.test(cls));
    ...
    addGolrTable(info, 'object_closure', 'gene-table', gene_filter, 'gene_anatomy', '#genes');
    info.geneNum = engine.fetchAssociationCount(gene_filter, 'subject');

    addNonEmptyAnchors(info);

    info.title = 'Monarch Anatomical Entity: '+info.label+' ('+ info.id+')';
    ...
    info.pup_tent_js_variables.push({name:'globalID',value:id});
    info.pup_tent_js_variables.push({name:'globalLabel',value:info.label});
    info.pup_tent_js_variables.push({name:'root',value:'UBERON:0001062'});
    info.classificationBrowserURL = 'http://uberon.github.io/';
    info.hasBrw = true;
    info.monarch_launchable.push("getOntologyBrowser(globalID, globalLabel, root)");
    ...
    info.node_logo = '/image/carousel-anatomy.png';
    return renderWithBase('node.mustache', info);
}
// Order matters here in the declaration of these routes.
defineRouteGET('/anatomy/{id}.{fmt}', ['id', 'fmt'], anatomyByIdHandler, errorResponse);
defineRouteGET('/anatomy/{id}', ['id'], anatomyByIdHandler, errorResponse);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/disease/{id}/{section}.{fmt}', ['id', 'section', 'fmt'], diseaseByIdSectionHandler, errorResponse);
defineRouteGET('/disease/{id}/{section}', ['id', 'section'], diseaseByIdSectionHandler, errorResponse);
...
async function geneByIdHandler(request, id, fmt) {
  console.log('geneByIdHandler', id, fmt);
    try {
        ...
        if (/_/.test(id)){
            engine.log("ID contains underscore, replacing with : and redirecting");
            var newID = id.replace("_",":");
            return renderRedirect(genURL('gene',newID));
        }

        // Rendering.
        var info = newInfo();
        info = await engine.fetchDataInfo(id);
        ...
        info.diseaseNum = await engine.fetchAssociationCount(disease_filter, 'object');
        ...
        info.homologNum = await engine.fetchAssociationCount(homolog_filter, 'object');
        ...
        info.orthoPhenoNum = await engine.fetchAssociationCount(ortho_phenotype_filter, 'object');

        var ortho_disease_filter = [
            { field: 'object_category', value: 'disease' },
            { field: 'subject_ortholog_closure', value: info.cliqueLeader }
        ];
        addGolrTable(info, 'subject_ortholog_closure', 'ortholog-disease-table', ortho_disease_filter,
                'gene_disease','#ortholog-diseases', false);

        info.orthoDiseaseNum = await engine.fetchAssociationCount(ortho_disease_filter, 'object');
        ...
        info.interactionNum = await engine.fetchAssociationCount(interaction_filter, null);
        ...
        addGolrTable(info, 'subject_closure', 'function-table',
                function_filter, 'gene_function', '#functions', false);

        info.functionNum = await engine.fetchAssociationCount(function_filter, 'object');
        ...
        addGolrTable(info, 'subject_closure', 'anatomy-table',
                anatomy_filter, 'gene_anatomy', '#anatomy', false);

        info.anatomyNum = await engine.fetchAssociationCount(anatomy_filter, 'object');
        ...
        info.primary_xref = function() {return genExternalHref('source', {id : id})};
        info.xrefs = function() {
            var result = buildXrefsString(id, info.database_cross_reference);
            // console.log('buildXrefsString', info.database_cross_reference, result);
            return result;
        };
        info.hasXrefs = function() {return checkExistence(info.database_cross_reference) || !!info.primary_xref};
        ...
        if (id.indexOf('HGNC:') === 0) {
            let url = `https://varsome.com/gene/${id}`;
            let varsomeLink = `<a href="${url}" target="_blank">Varsome</a>`;
            info.equivalentClasses.push(varsomeLink);
        }
        info.altids = function() {
            if (info.has_alternative_id != null) {
                return info.has_alternative_id.join(", ");
        ...
        info.includes.exac_gene_summary = expandTemplate('exac_gene_summary', info);
        info.monarch_launchable.push('createExaxGeneSummaryTable(globalID)');
...
// Order matters here in the declaration of these routes.
defineRouteGET('/gene/{id}.{fmt}', ['id', 'fmt'], geneByIdHandler, errorResponse);
defineRouteGET('/gene/{id}', ['id'], geneByIdHandler, errorResponse);
...
var redirectDiseasePage = async function(id, equivalentNodes) {
    ...
    if (prefix != 'OMIM' && 'OMIM' in nodeMap && nodeMap['OMIM'].lbl != null){
        return renderRedirect(genURL('disease',nodeMap['OMIM'].id));

    } else if (prefix != 'DOID' && 'DOID' in nodeMap && nodeMap['DOID'].lbl != null){
        return renderRedirect(genURL('disease',nodeMap['DOID'].id));

    } else if (prefix != 'Orphanet' && 'Orphanet' in nodeMap && nodeMap['Orphanet'].lbl != null){
        return renderRedirect(genURL('disease',nodeMap['Orphanet'].id));

    } else if (prefix != 'MESH' && 'MESH' in nodeMap && nodeMap['MESH'].lbl != null){
        return renderRedirect(genURL('disease',nodeMap['MESH'].id));
    }
};


//Note there is much copied from the above function, should refactor to not repeat code
async function variantByIdHandler(request, id, fmt) {
    try {

        if (/_/.test(id) && !/\:/.test(id)){
            engine.log("ID contains underscore, replacing with : and redirecting");
            var newID = id.replace("_",":");
            return renderRedirect(genURL('variant',newID));
        }
        ...
        info.pup_tent_js_variables.push({name: 'varID', value: id,});
        info.includes.exac_table = expandTemplate('exac_table', info);
        info.monarch_launchable.push('createExacTable(varID)');
        ...
        info.geneNum = await engine.fetchAssociationCount(gene_filter, 'object');
        ...
        info.genotypeNum = await engine.fetchAssociationCount(genotype_filter, 'object');
...
// Order matters here in the declaration of these routes.
defineRouteGET('/variant/{id}.{fmt}', ['id', 'fmt'], variantByIdHandler, errorResponse);
defineRouteGET('/variant/{id}', ['id'], variantByIdHandler, errorResponse);

//Sequence Feature - Sub-pages
//Example: /gene/NCIBGene:12166/phenotype_associations.json
//Currently only works for json or rdf output

defineRouteGET('/gene/{id}/{section}.{fmt?}', ['id', 'section', 'fmt'], fetchFeatureSection);
defineRouteGET('/variant/{id}/{section}.{fmt?}', ['id', 'section', 'fmt'], fetchFeatureSection);
defineRouteGET('/genotype/{id}/{section}.{fmt?}', ['id', 'section', 'fmt'], fetchFeatureSection);
defineRouteGET('/model/{id}/{section}.{fmt?}', ['id', 'section', 'fmt'], fetchFeatureSection);
defineRouteGET('/case/{id}/{section}.{fmt?}', ['id', 'section', 'fmt'], fetchFeatureSection);
        ...
        try {
          ...
          //Curify ID if needed
            if (/_/.test(id)){
                engine.log("ID contains underscore, replacing with : and redirecting");
                var newID = id.replace("_",":");
                return renderRedirect(genURL('association',newID));
            }
            ...
            return renderWithBase('associations.mustache', info);
        } catch(err) {
            return errorResponseWithObject(err);
        }
}
...
// Order matters here in the declaration of these routes.
defineRouteGET('/association/{id}.{fmt}', ['id', 'fmt'], associationByIdHandler, errorResponse);
defineRouteGET('/association/{id}', ['id'], associationByIdHandler, errorResponse);
...
function addOrthoDiseaseAnchor(info) {
    var ortho_disease_anchor = {id: info.id,
                            resultNum: info.orthoDiseaseNum,
                            type: "Ortholog-Diseases", href: "ortholog-diseases"};
    return expandTemplate('anchor', ortho_disease_anchor);
}

function addOrthoDiseaseTable() {
    var ortho_disease_table = {href: "ortholog-diseases", div: "ortholog-disease-table", filter: true};
    return expandTemplate('golr-table', ortho_disease_table);
}
...
function addFunctionAnchor(info) {
    const function_anchor = {
        id: info.id,
        resultNum: info.functionNum,
        type: 'Functions', href: 'functions'
    };
    return expandTemplate('anchor', function_anchor);
}

function addFunctionTable() {
    const function_table = {
        href: 'functions', div: 'function-table', filter: false
    };
    return expandTemplate('golr-table', function_table);
}

function addAnatomyAnchor(info) {
    const anatomy_anchor = {
        id: info.id,
        resultNum: info.anatomyNum,
        type: 'Anatomy',
        href: 'anatomy'
    };
    return expandTemplate('anchor', anatomy_anchor);
}

function addAnatomyTable() {
    const anatomy_table = {
        href: 'anatomy', div: 'anatomy-table', filter: false
    };
    return expandTemplate('golr-table', anatomy_table);
}
...
function analyzeByDatatypePostHandler(request, datatype, fmt) {
    ...
    return renderWithBase('analyze.mustache', info);
}

async function analyzeByDatatypeGetHandler(request, datatype, fmt) {
    ...
    console.log('analyzeByDatatypeGetHandler   target, species, mode, limit, user_results, input_items: ', target, species, mode, limit, user_results, input_items);
     ...
     if (input_items != null) {
         ...
         for (var spn = 0; spn < input_items.length; spn++) {
             if (input_items[spn]){
                console.log('before fetchClassInfo', spn, input_items[spn]);
                var phenoInfo = await engine.fetchClassInfo(input_items[spn],{level:0})
                console.log('after fetchClassInfo', spn, input_items[spn], phenoInfo);
                 if (phenoInfo.label) {
                     if (splitLabels){
         ...
         const inputItemsArray = await engine.mapIdentifiersToPhenotypes(input_items);
         input_items = inputItemsArray.join().split(',');
         // engine.log("input items...."+JSON.stringify(input_items));
         // engine.log("# of input items..."+input_items.length);

         var resultObj = await engine.searchByAttributeSet(input_items, tf, limit);
...
//web.wrapRoutePost(app, '/analyze/{datatype}/', ['datatype', 'fmt'], analyzeByDatatypePostHandler, errorResponse);
defineRouteGET('/analyze/{datatype}.{fmt}', ['datatype', 'fmt'], analyzeByDatatypeGetHandler, errorResponse);
defineRouteGET('/analyze/{datatype}/', ['datatype'], analyzeByDatatypeGetHandler, errorResponse);
defineRouteGET('/analyze/{datatype}', ['datatype'], analyzeByDatatypeGetHandler, errorResponse);
...
web.wrapRouteGet(app, '/favicon.ico', [], faviconHandler, errorResponse);

async function resolveByIdHandler(request, id, fmt) {
    var result;
    ...
    var type = await engine.resolveIdToType(id);
    if (type) {
        var redirectURL = genURL(type, id);
        result = renderRedirect(redirectURL);
    } else {
...
async function rootPathHandler(request, id, fmt) {
  console.log('rootPathHandler', id, fmt);
    var result;
    ...
    if (id.indexOf('apple-touch-icon') === 0) {
        ...
        var type = await engine.resolveIdToType(id);
        if (type) {
            result = renderRedirect(genURL(type, id));
        }
        else {
            // engine.log('Unable to resolve id: ' + id);
            result = notFoundResponse('Unable to resolve id: ' + id);
...
// Order matters here in the declaration of these routes.
defineRouteGET('/resolve/{id}.{fmt}', ['id', 'fmt'], resolveByIdHandler, errorResponse);
defineRouteGET('/resolve/{id}', ['id'], resolveByIdHandler, errorResponse);

defineRouteGET('/{id}.{fmt}', ['id', 'fmt'], rootPathHandler, errorResponse);
defineRouteGET('/{id}', ['id'], rootPathHandler, errorResponse);
...
defineRouteGET('/downloads/{id}.{fmt}', ['id', 'fmt'], downloadByIdHandler, errorResponse);
...
app.startServer = async function() {
    ...
    console.log('HapiJS Server running at:', app.info.uri);
