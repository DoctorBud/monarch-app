...
function wrapResponse(request, h, output) {
    console.log('wrapResponse', JSON.stringify(output, null, 2).slice(0, 30));
    var response = h.response(output.body);
...
function wrapRouteGet(app, hapiPath, props, commonHandler, errorHandler) {
    var that = this;
    async function wrappedHandler(request, h) {
        var     vals = [request];
        for (var propIndex in props) {
            vals.push(request.params[props[propIndex]]);
        }

        var output = await commonHandler.apply(that, vals);
        return wrapResponse(request, h, output);
    }
