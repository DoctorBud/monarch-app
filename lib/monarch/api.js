...
var logGOLR = env.getEnv().USE_LOG_GOLR;
if (logGOLR && logGOLR === '1') {
    console.log('#USE_LOG_GOLR enabled');
    logGOLR = true;
}
else {
    logGOLR = false;
...
bbop.monarch.Engine = function(opts) {
    ...
    if (this.config.owlsim_services_url == null) {
        this.config.owlsim_services_url = "https://monarchinitiative.org/owlsim";
    }
    ...
    this.fetchUrlImplementation = async function(url) {
        var requestResult = await AsyncRequest.get(url);
...
bbop.monarch.Engine.prototype.fetchTaxon = async function(id) {
    var engine = this;

    var leader = await engine.fetchCliqueLeader(id, engine.config.scigraph_data_url);
    var neighbors = await engine.getGraphNeighbors(leader, 1, 'RO:0002162', 'BOTH', false, engine.config.scigraph_data_url);
    var graph = new bbop.model.graph();
    graph.load_json(neighbors);
    var tax = {};
    var taxon_list = graph.get_parent_nodes(leader, 'RO:0002162');
    if (taxon_list){
...
bbop.monarch.Engine.prototype.fetchOrthologList = async function(ids) {
    var engine = this;
    ...
    for (const id of ids) {
        var orthoObj = await engine.fetchGeneOrthologFromSciGraph(id,'label',true);
 ...
 *
 */
bbop.monarch.Engine.prototype.fetchGeneOrthologFromSciGraph = async function(id) {
    var engine = this;

    var CLASS_MAP = {
            'RO:HOM0000017' : 'Ortholog',
            'RO:HOM0000020' : 'Least Diverged Ortholog',
            'RO:HOM0000019': 'Homolog',
            'RO:HOM0000011' : 'Paralog',
            'RO:HOM0000023' : 'Paralog',
            'RO:HOM0000022' : 'Ohnolog',
            'RO:HOM0000018': 'Xenolog',
            'RO:0002351' : 'has_member'
    };
    ...
    var scigraphResults = await engine.querySciGraphDynamicServices(path,params);
...
bbop.monarch.Engine.prototype.fetchInheritance = async function (id) {
    const engine = this;
    const hasDisposition = 'RO:0000091';
    const params = {
        depth: 1,
        relationshipType: hasDisposition,
        direction: 'OUTGOING',
        blankNodes: false
    };

    const neighbors = await engine.getGraphNeighbors(
        id, params.depth,
        params.relationshipType, params.direction,
        params.blankNodes, engine.config.scigraph_data_url
    );

    const neighorGraph = new bbop.model.graph();
    neighorGraph.load_json(neighbors);

    return neighorGraph.get_parent_nodes(id, hasDisposition);
};
 ...
 *
 */
bbop.monarch.Engine.prototype.getInformationProfile = async function(annotation_profile,categories) {
    var engine = this;
    var score = 0.00;
    var scored_profile = {};
    ...
    var resultStr = await this.fetchUrl(url,
            { a : atts,
              r : categories },
            'post');
    if (atts == []) {
    ...
    if (nots.length > 0) {
        resultStr = await this.fetchUrl(url,
                { a : nots,
                  r : categories },
                  'post');
...
bbop.monarch.Engine.prototype.getMinScorePerCategory = async function(categories) {
    var engine = this;
    var categorical_scores = [];
    for (const c in categories) {
        var url = engine.config.owlsim_services_url + '/getAttributeInformationProfile';
        var resultStr = await engine.fetchUrl(url,
                        { a : [c],
                          r : categories },
                          'post');
        var att_info =  JSON.parse(resultStr);
 ...
 *
 */
bbop.monarch.Engine.prototype.searchByPhenotypeProfile = async function(query,target_species,target_type,cutoff,metric) {
    var engine = this;
    ...
    simResults = await engine.searchByAttributeSet(id_list,target,cutoff);
    monarchSimResults = engine.makeSimComparisonResults(simResults.results,metric,target_type,target_species);
 ...
 */
bbop.monarch.Engine.prototype.compareEntities = async function(x,y) {
    var engine = this;
    var defaultMetric = 'combinedScore';
    ...
    var xphenotype_ids = await engine.getPhenotypes(x);
    ...
    for (var it=0; it<y.length; it++) {
        var yphenotype_ids = await engine.getPhenotypes(y[it]);

        var stuff = {};

        var resultObj = await engine.fetchAttributeComparisonMatrix(xphenotype_ids,yphenotype_ids);
        ...
        var node = await engine.getGraphNodeByID(target_id, engine.config.scigraph_data_url);
        var graph = new bbop.model.graph();
        ...
        stuff.taxon = await engine.fetchTaxon(target_id);

        results.push(stuff);
    }
    var comp = await engine.makeSimComparisonResults(results,defaultMetric);

    //TODO abstract this out to a function!
    var a = {};
    if (x.length == 1) {
        var n = await engine.getGraphNodeByID(x[0], engine.config.scigraph_data_url);
        var graph = new bbop.model.graph();
        graph.load_json(n);
        ...
        a.taxon = await engine.fetchTaxon(x[0]);  //TODO replace with a scigraph call
    } else {
        var xlabels = [];
        for (const xid in x) {
            var n = await engine.getGraphNodeByID(xid, engine.config.scigraph_url);
            var graph = new bbop.model.graph();
            graph.load_json(n);
...
bbop.monarch.Engine.prototype.getCategories = async function(id) {
    var engine = this;
    var categories = [];
    var nodeByID = await engine.getGraphNodeByID(id, engine.config.scigraph_data_url);
    var graph = new bbop.model.graph();
 ...
 */
bbop.monarch.Engine.prototype.resolveIdToType = async function(id) {
    var engine = this;
    ...
    if (node) {
        ...
        if (metadata && metadata['category']) {
            ...
            if (metadata['category'].indexOf('disease') > -1) {
                ...
                type = 'literature';
            } else if (metadata['category'].indexOf('anatomical entity') > -1) {
                type = 'anatomy';
 ...
 *
 */
bbop.monarch.Engine.prototype.searchByAttributeSet = async function(atts, tf, limit) {
    ...
    var resultStr = await this.fetchUrl(this.config.owlsim_services_url + '/searchByAttributeSet',
            ph,
            'post');
...
bbop.monarch.Engine.prototype.fetchAttributeComparisonMatrix = async function(x,y) {
    ...
    var resultStr = await this.fetchUrl(this.config.owlsim_services_url + '/compareAttributeSets',
            {
                a : x,
                b : y,
            },
...
bbop.monarch.Engine.prototype.fetchAttributeMultiComparisonMatrix = function(x,model_ids) {
    ...
    var disease = this.fetchDiseaseInfo(x); // There is no fetchDiseaseInfo
...
bbop.monarch.Engine.prototype.mapIdentifiersToPhenotypes = async function(ids) {
    var pids = [];
    var exclude_pids = [];
    for (var k in ids) {
        var pid = await this.expandIdentifierToPhenotype(ids[k]);
        if (pid.indexOf("-") == 0) {
...
bbop.monarch.Engine.prototype.expandIdentifierToPhenotype = async function(id) {
    var engine = this;
    id = id.replace("_",":");
    var parts = id.split(":");
    var db = parts[0];
    ...
    var idType = this.mapIdentifierType(id);
    ...
    if ( idType == 'disease') {
        ...
        resultObj = await this.fetchOmimDiseasePhenotypeAsAssocations(id);
    ...
    if (idType == 'gene' || idType == 'genotype') {
        if (idType == 'gene') {
            console.log('#expandIdentifierToPhenotype() type "gene" not implemented', id, idType);
            ...
            resultObj = await engine.fetchGenotypeInfo(id.trim());
 ...
 *
 */
bbop.monarch.Engine.prototype.getPhenotypes = async function(id_list) {
    ...
    for (var i=0; i<id_list.length; i++) {
        var id = id_list[i];
        var categories = await engine.getCategories(id);
        if (categories.indexOf('phenotype') > -1) {
            expanded_list.push(id);
        } else {
            var phenotype_list = await engine.fetchPhenotypes(id);
 ...
 */
bbop.monarch.Engine.prototype.fetchClassInfo = async function(id, opts) {

    var resultObj = {};
    var engine = this;

    var res = await engine.getVocabularyTermByID(id);
    ...
    var nodeByID = await engine.getGraphNodeByID(id);
    ...
    if (opts != null) {
        if (opts.level != null && opts.level > 0) {
            ...
            var neighbors = await engine.getGraphNeighbors(id,opts.level);
            var triples = [];
...
bbop.monarch.Engine.prototype.annotateText = async function(txt, opts) {
    var engine = this;
    var q = opts;
    q.content = txt;
    ...
    var res =
        await this.fetchUrl(annotate_url, q);
    ...
    var neighbors = await engine.getGraphNeighbors(id_list, 1, 'RO:0002162', 'BOTH', false, engine.config.scigraph_data_url);
    var graph = new bbop.model.graph();
    graph.load_json(neighbors);
    id_list.forEach(function (id) {
        var label = '';
        var taxon_list = graph.get_parent_nodes(id, 'RO:0002162');
...
bbop.monarch.Engine.prototype.golrHackForSearch = function(golrManager) {
    ...
    golrManager.add_query_field("taxon_label_std", 1.0);
    golrManager.add_query_field("taxon_label_kw", 1.0);
    golrManager.add_query_field("taxon_label_eng", 1.0);

    golrManager.add_query_field("taxon_label_synonym_std", 1.0);
    golrManager.add_query_field("taxon_label_synonym_kw", 1.0);
    golrManager.add_query_field("taxon_label_synonym_eng", 1.0);
}
 ...
 */
bbop.monarch.Engine.prototype.search = async function(term, filters, facets, rows, start) {
    var engine = this;
    ...
    return new Promise((resolve, reject) => {
        // reject(new Error('ABORTING ES6Promise from .search without .page'));
        // resolve({bogus: 'OBJECT'});

        searchGolrManager.page(rows, start)
            .then(r => {
                // TODO DELETE THIS WHEN #1387 is done
                ...
                var hl = _.map(zipped, function(tuple) {
                    ...
                    return _.mapObject(json, function(val, key) {
                        ...
                        _.each(postfixes, function(postfix) {
                            ...
                            if(highlight[key + postfix] != null && key != 'id') {
            ...
            .fail(function(err) {
                console.log('failing ES6Promise from .search', err);
                if (err) {
                    reject(err);
                }
            }).done();
 ...
 * Returns: JSON structure
 */
 bbop.monarch.Engine.prototype.autocomplete = async function(term, category, limit) {
     var engine = this;
    ...
    searchGolrManager.set_query(term);
    searchGolrManager.add_query_filter("prefix","OMIA", "-");
 ...
 * Returns: Class struct as JSON
 */
bbop.monarch.Engine.prototype.searchOverOntologies = async function(term, opts) {
    var engine = this;
    var results = [];
    var searchSynonyms = true;
    var categories = await engine.getDefaultCategories();
    ...
    var sciObj = await engine.getVocabularyByTerm(term,searchType,limit,categories,searchSynonyms);
    ...
    try {
        var neighbors = await engine.getGraphNeighbors(id_list, 1, 'RO:0002162',
                'BOTH', false, engine.config.scigraph_url);
        var graph = new bbop.model.graph();
        graph.load_json(neighbors);
        results = await engine.addTaxonToGraph(results, id_list, graph);
    } catch(err) {
    ...
    var eq_graph = await engine.getGraphNeighbors(id_list, 5, 'equivalentClass', 'BOTH', false, engine.config.scigraph_url);
 ...
 */
bbop.monarch.Engine.prototype.fetchUrl = async function(url, params, method) {
    var exchangeObj = await this.fetchUrlWithExchangeObject(url, params, method);
...
bbop.monarch.Engine.prototype.fetchUrlWithExchangeObject = async function(url, params, method) {
    var data = '';
    if (params == null) {
        params = {};
    }
    ...
    var exchangeObj = {
        ...
        content: null    // res.body.toString('utf-8')
    };
    try {
        var res;
        if (method === 'post') {
            ...
            res = await AsyncRequest.post(postOptions);
        }
        else {
            var getOptions = {
                url: url,
            ...
            res = await AsyncRequest.get(getOptions);
        }
        exchangeObj = {
            status: 200,    // res.statusCode,
            content: res    // res.body.toString('utf-8')
        };
    }
    catch (e) {
        // console.log('fetchUrlWithExchangeObject EXCEPTION', e);
        exchangeObj = {
            status: 404,    // res.statusCode,
            content: e.toString()    // res.body.toString('utf-8')
        };
    }
    ...
    if (logFetchTimes) {
        ...
        this.log("FETCHED: " + method + " " + url + ' params=' + JSON.stringify(params) +
            ' status: ' + exchangeObj.status + "  Length: " + exchangeObj.content.length + "  Time: " +
            timeDelta + "ms" +
...
bbop.monarch.Engine.prototype.getEquivalentClassList = async function(id, server) {
    var engine = this;
    if (!id){
        return null;
    }
    ...
    var neighbors = await engine.getGraphNeighbors(id, 1, relationshipType, 'BOTH', false, server);
...
//
bbop.monarch.Engine.prototype.getGraphNeighbors = async function(id, depth, relationshipType, direction, blankNodes, server) {
    ...
    var resultObj = [];
    resultObj = await this.querySciGraphGraphServices(path,params, server);
    return resultObj;
}
...
bbop.monarch.Engine.prototype.getVocabularyTermByID = async function(id, server) {

    var params = null; //No parameters for this operation
    var path = "vocabulary/id/"+id+'.json';
    var resultObj = [];

    return await this.querySciGraphVocabServices(path, params, server);
}
...
bbop.monarch.Engine.prototype.getGraphNodeByID = async function(id, server) {
    var engine = this;
    var params = null; //No parameters for this operation
    ...
    //Fetch URI
    var exchangeObj = await engine.fetchUrlWithExchangeObject(uri,params);
...
bbop.monarch.Engine.prototype.getVocabularyByTerm = function(term,searchType,limit,category,searchSynonyms,ontology) {
    ...
    return engine.querySciGraphVocabServices(path,params);
}
...
bbop.monarch.Engine.prototype.querySciGraphGraphServices = async function(path, params, server) {
    ...
    return JSON.parse(exchangeObj.content);
}

bbop.monarch.Engine.prototype.querySciGraphVocabServices = async function(path, params, server) {
    ...
    try {
        ...
        console.log('Error during fetchUrlWithExchangeObject', uri, params, e);
    }
}

bbop.monarch.Engine.prototype.querySciGraphDynamicServices = async function(path,params) {
...
bbop.monarch.Engine.prototype.initializeGolrManager = function(personality, golrServer) {
    ...
    var async_engine = require('bbop-rest-manager').node;
    var golr_manager = require('bbop-manager-golr');
    var golr_conf = require('golr-conf');
    var engine_to_use = new async_engine(golr_response);
    ...
    var golrManager = new golr_manager(golrServer, golrConf, engine_to_use, 'async');
    const broken_get_query_url = golrManager.get_query_url.bind(golrManager);
    golrManager.get_query_url = function() {
        const badURL = broken_get_query_url();
        return badURL.replace('/select?', '/select/?');
    }
    if (logGOLR) {
        golrManager.debug(true);
    }
    if(personality != null) {
 ...
 *
 */
bbop.monarch.Engine.prototype.fetchAssociations = async function(filters, limit, personality, facet, orFilter) {
    var engine = this;

    var golrManager = await engine.initializeGolrManager(personality);
    ...
    return new Promise((resolve, reject) => {
        var searchResult = golrManager.search();
        searchResult
            .then(result => {
                resolve(result);
            })
            .fail(function(err) {
                console.log('golrManager.search FAIL', err);
                if (err) {
                    reject(err);
                }
            }).done();
...
bbop.monarch.Engine.prototype.fetchAssociationCount = async function(filter, facet, orFilter) {
    var golrResponse = await this.fetchAssociations(filter, 0, null, facet, orFilter);
    var count = 0;
    if (facet != null){
...
bbop.monarch.Engine.prototype.fetchCoreDiseaseInfo = function(id) {
    ...
    const obj = this.fetchClassInfo(id, { level: 1 });
 ...
 *
 */
bbop.monarch.Engine.prototype.fetchGenotypes = async function(id) {
    var engine = this;

    //Get phenotype associations
    var filters = [
                  {
                      field: 'object_closure',
                      value: id
                  },
                  {
                      field: 'subject_category',
    ...
    var golrResponse = await engine.fetchAssociations(filters, 5000);
 ...
 *
 */
bbop.monarch.Engine.prototype.fetchPhenotypes = async function(id) {
    var engine = this;
    ...
    const personality = 'gene_phenotype'
    var golrResponse = await engine.fetchAssociations(filters, 5000, personality);
...
bbop.monarch.Engine.prototype.fetchCliqueLeader = async function(id, server) {
    var engine = this;
    var leader = null;
    ...
    var uri = server + path;
    var exchangeObj = await engine.fetchUrlWithExchangeObject(uri);
...
bbop.monarch.Engine.prototype.fetchDataInfo = async function(id, opts) {

    var resultObj = {};
    var engine = this;
    var server = engine.config.scigraph_data_url;
    var res = await engine.getVocabularyTermByID(id, server);
    //TODO do we need to handle lists of >1
    resultObj = {};
    if (typeof res.concepts !== 'undefined' &&
            typeof res.concepts[0] != 'undefined'){
    ...
    var nodeByID = await engine.getGraphNodeByID(id, server);
    var xrefs = "http://www.geneontology.org/formats/oboInOwl#hasDbXref";
    ...
    var neighbors = await engine.getGraphNeighbors(id, 1, undefined, 'BOTH', false, server);
    var graph = new bbop.model.graph();
    graph.load_json(neighbors);

    var equivalentNodes = await engine.getEquivalentNodes(id, graph);
    resultObj.equivalentNodes = equivalentNodes;
    resultObj.equivalentClasses = equivalentNodes.map(function (i) { return i.id; });
    ...
    if (taxon_list == null || taxon_list.length == 0){
        taxon_list = graph.get_parent_nodes(id, 'RO:0002162');
    }
    ...
    resultObj.cliqueLeader = await engine.fetchCliqueLeader(id);
...
bbop.monarch.Engine.prototype.getGolrDownloadUrl = function (id, format, association) {
    ...
    if (typeof association === 'undefined') {
        ...
        field_list = ['subject', 'subject_label', 'subject_category',
                      ...
                      'subject_gene', 'subject_gene_label','relation',
                      'object', 'object_gene', 'object_gene_label',
