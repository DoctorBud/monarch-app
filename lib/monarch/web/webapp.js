...
function searchHandler(request, term) {
    console.log('searchHandler', request, term);
    try {
...
function diseaseLandingHandler(request) {
    console.log('diseaseLandingHandler');
...
if (useSPA) {
  function computeCounts(info, nodetype, id) {
  ...
  function nodeByTypeIdHandler(request, nodetype, id) {
      ...
      // Rendering.
      var info = newInfo(engine.fetchClassInfo(id, {level:1}));
      console.log('info', nodetype, id);

      computeCounts(info, nodetype, id);
      ...
      const cliqueLeader = engine.fetchCliqueLeader(id, engine.config.scigraph_data_url);
      ...
      // TODO remove after scigraph is loaded with inheritance and clinical
      // modifier categories
      // See https://github.com/monarch-initiative/SciGraph-docker-monarch-data/
      // commit/43f0ab89cc4a8d93fe93e5a6a0faa5844e0c0ebb
  ...
  /*
      ...
      const cliqueLeader = engine.fetchCliqueLeader(id);
      const inheritanceList = engine.fetchInheritance(cliqueLeader);
...
function diseaseByIdHandler(request, id, fmt) {
    console.log('diseaseByIdHandler', id, fmt);
...
function geneByIdHandler(request, id, fmt) {
  console.log('geneByIdHandler', id, fmt);
    try {
        ...
        info.orthoDiseaseNum = engine.fetchAssociationCount(ortho_disease_filter, 'object');
        ...
        info.functionNum = engine.fetchAssociationCount(function_filter, 'object');
        ...
        info.anatomyNum = engine.fetchAssociationCount(anatomy_filter, 'object');
        ...
        info.primary_xref = function() {return genExternalHref('source', {id : id})};
        info.xrefs = function() {
            var result = buildXrefsList(id, info.database_cross_reference);
            console.log('buildXrefsList', info.database_cross_reference, result);
            return result;
        };
        info.hasXrefs = function() {return checkExistence(info.database_cross_reference) || !!info.primary_xref};
...
function variantByIdHandler(request, id, fmt) {
    try {
        ...
        info.pup_tent_js_variables.push({name: 'var_id', value: id,});
        info.includes.exac_table = expandTemplate('exac_table', info);
        info.monarch_launchable.push('createExacTable(var_id)');
