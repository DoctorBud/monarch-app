          ...
          app.route({
            ...
            handler:
              function (request, reply) {
                console.log('###pup_tent GET', thing);
...
function staticTemplate(t) {
    var info = newInfo();
    addCoreRenderers(info);
    console.log('#pup_tent.render1', t+'.mustache');
...
function expandTemplate(tmplName, tmplInfo) {
    console.log('#pup_tent.apply', 'templates/' +  tmplName + '.mustache');
...
function errorResponse(errorStatus, errorMessage) {
    ...
    console.log('#pup_tent.render2', template);
...
web.wrapGet(app, '/status', statusHandler, errorResponse,
    {
        description: 'Returns JSON with various server statistics',
        notes:
`
<pre>
Example Output:
{
  ...
  "name": "Monarch Application",
  "okay": true,
  "message": "okay",
  "date": "Fri Dec 23 2016 07:11:34 GMT-0800 (PST)",
  "platform": "nodejs",
  "platform_version": "NodeJS: v4.5.0 HapiJS: 15.0.3",
  "offerings": [
    {
      "name": "api_version",
      "value": "monarch-api-2014-12-19"
    },
    {
      "name": "config_type",
      "value": "dev"
...
function serviceStatusHandler(request) {
    ...
    console.log('#pup_tent.render3', 'status.mustache');
...
function pageByPageHandler(request, page) {
    ...
    if (info.title) {
        console.log('#pup_tent.render4', page + '.mustache');
...
web.wrapStatic(app, '/docs/{file}', './docs/');
web.wrapStatic(app, '/docs/files/{file}', './docs/files/');
web.wrapStatic(app, '/docs/files/web/{file}', './docs/files/web/');
web.wrapStatic(app, '/docs/styles/{file}', './docs/styles/');
web.wrapStatic(app, '/docs/index/{file}', './docs/index/');
web.wrapStatic(app, '/docs/javascript/{file}', './docs/javascript/');
...
//     }, errorResponse
// );
web.wrapStatic(app, '/fonts/fontawesome{suffix}', './node_modules/font-awesome/fonts/fontawesome');
web.wrapStatic(app, '/fonts/glyphicons{suffix}', './node_modules/bootstrap/dist/fonts/glyphicons');
web.wrapStatic(app, '/fonts/{file}', './fonts/');
...
//     }, errorResponse
// );
web.wrapStatic(app, '/image/{image}', './image/');
...
//     }, errorResponse
// );
web.wrapStatic(app, '/image/team/{image}', './image/team/');
...
//     }, errorResponse
// );
web.wrapStatic(app, '/images/{jquifile}', './node_modules/jquery-ui/themes/base/images/');
...
//     }, errorResponse
// );
web.wrapStatic(app, '/node_modules/phenogrid/{filename}', './node_modules/phenogrid/');
web.wrapStatic(app, '/node_modules/phenogrid/config/{filename}', './node_modules/phenogrid/config/');
web.wrapStatic(app, '/node_modules/phenogrid/dist/{filename}', './node_modules/phenogrid/dist/');
web.wrapStatic(app, '/node_modules/phenogrid/dist/fonts/fontawesome-{fafile}', './node_modules/font-awesome/fonts/fontawesome-');
...
function searchFilteringHandler(request, term, filters, pageNum) {
    console.log('searchHandler: term: ', term);
    console.log('searchHandler: filters: ', filters);
    console.log('searchHandler: pageNum: ', pageNum);

    try {
        // Only display 30 results per page start from row 1, index 0
        // This can be used for paging
        var resultsCountPerPage = 25;
        //var results = engine.autocomplete(term, null, resultsCount);
        
        // Only faceting with category and taxon_label - Zhou
        // Remember to convert JSON string to JSON object
        var golrResults = engine.search(term, JSON.parse(filters), ["category", "taxon_label"], resultsCountPerPage, (pageNum - 1)*resultsCountPerPage);
        ...
        // Default to true to show the load more button
        var loadMoreBtn = true;

        // Handle the golr JSON using bbop-response-golr
        var bgr = new bbop_response_golr(golrResults);

        // object with rendering functions
        if (bgr.documents().length > 0) {
            count = bgr.total_documents();

            // Decide whether to show the load more button
            loadMoreBtn = true;
            if (resultsCountPerPage*pageNum >= count) {
                loadMoreBtn = false;
            } 

            // Display rows of results that are showing
            var startRow = (pageNum - 1)*resultsCountPerPage + 1;
            var endRow = (pageNum - 1)*resultsCountPerPage + resultsCountPerPage;
            if (endRow > count) {
                endRow = count;
            }
            ...
            // This is an array
            //var highlighted_docs = bgr.highlighted_documents();
            // use documents() as tmp hack - according to Jeremy
            var highlighted_docs = bgr.documents();
            //console.log(highlighted_docs);

            for (var i = 0; i < highlighted_docs.length; i++) {
                // Temp workaround since some results don't have taxon info - Zhou
                var taxonText;

                if (typeof(highlighted_docs[i].taxon_label) !== 'undefined') {
                    taxonText = highlighted_docs[i].taxon_label;
                } else {
                    taxonText = '';
                } 

                // Use the first highlighted string since no better/simplier approach 
                // after discussion with Jeremy and Seth- Zhou
                var matchingStr = '';
                if (typeof(highlighted_docs[i].synonym) !== 'undefined') {
                    matchingStr = highlighted_docs[i].synonym[0];
                } else if (typeof(highlighted_docs[i].definition) !== 'undefined') {
                    matchingStr = highlighted_docs[i].definition[0];
                } else {
                    // Just use the label if highlighted?
                    matchingStr = highlighted_docs[i].label;
                }

                // Convert all category elements to lowercase since genObjectHref() doesn't render "Phenotype"
                // correctly in the URL - Zhou
                var categoryArr = highlighted_docs[i].category.map(function(val) {
                    return val.toLowerCase();
                });

                // Only use label_searchable for now - Zhou
                resultsOutput += '<tr class="search-result-item">'
                    + '<td>' + genObjectHref(categoryArr, highlighted_docs[i]) +'</td>'
                    + '<td>' + categoryArr.join(',') + '</td>'
                    + '<td>' + taxonText + '</td>'
                    + '<td>' + matchingStr + '</td>'
                    + '</tr>';
            }
...
web.wrapRouteGet(app, '/searchfiltering/:term/:filters/:pageNum', '/searchfiltering/{term}/{filters}/{pageNum}', ['term', 'filters', 'pageNum'], searchFilteringHandler, errorResponse);
...
function searchHandler(request, term) {
    ...
    try {
       ...
       //     var type = engine.resolveIdToType(term);
       //     url = genURL(type,term);
       ...
       // }
        ...
        // Only faceting with category and taxon_label - Zhou
        var golrResults = engine.search(term, {}, ["category", "taxon_label"], resultsCountPerPage, 0);

        var info = newInfo();
        info.results = golrResults;

        if (fmt != null) {
            // Comment out these lines in order to see the highlighted results for debugging - Zhou
            // var bgr = new bbop_response_golr(golrResults);
            // var t = bgr.highlighted_documents();
            // console.log(t);

            // return formattedResults(t, fmt);
        ...
        if (bgr.documents().length > 0) {
            ...
            info.count = bgr.total_documents();

            // Decide whether to show the load more button
            info.loadMoreBtn = true;
            if (resultsCountPerPage >= info.count) {
                info.loadMoreBtn = false;
            } 

            // Combination filters

            // No sorting, just use the default order
            var categories = info.results.facet_counts.facet_fields.category;

            // Sorting: human, mouse, zebrafish, fly, worm, and then everything else by data richness
            var taxa = info.results.facet_counts.facet_fields.taxon_label;
            ...
            // Create the new output format - Zhou
            info.resultsOutput = '<table class="search-results-table table table-striped table-condensed simpletable">'
                + '<caption> showing 1 - ' + resultsCountPerPage + ' of ' + info.count + ' results for "' + term + '"</caption>'
                + '<thead><tr><th width="25%">Term</th><th width="15%">Category</th><th width="25%">Taxon</th><th>Matching String</th></tr></thead>'
                + '<tbody>';
            ...
            for (var i = 0; i < highlighted_docs.length; i++) {
                ...
                // Only use label_searchable for now - Zhou
                info.resultsOutput += '<tr class="search-result-item">'
                    + '<td>' + genObjectHref(categoryArr, highlighted_docs[i]) +'</td>'
                    + '<td>' + categoryArr.join(',') + '</td>'
                    + '<td>' + taxonText + '</td>'
                    + '<td>' + matchingStr + '</td>'
            ...
            info.resultsOutput += '</tbody></table>';
        } else {
            ...
            var suggestionOutput = '';
            suggestions.forEach( function (suggestion) {
                var encodedSuggestion = encodeURI(suggestion);
                var href = "<a href=\"/search/" + encodedSuggestion +
                           "\">" + suggestion + "</a>";
                suggestionOutput += href + "</br>";
            });

            if (info.suggestSpan !== '') {
                info.suggestSpan = "<h4>No results found, did you mean:</h4>" + suggestionOutput;
            } else {
                info.suggestSpan = "<h4>No results found</h4>";
            }
        }
        ...
        // Make variable info available in search_results.js
        info.pup_tent_js_variables.push({name:'searchResults', value: info.results});
        ...
        console.log('#pup_tent.render5', 'search_results.mustache');
...
function sourcesHandler(request, fmt) {
    try {
        ...
        console.log('#pup_tent.render5', 'sources.mustache');
...
if (useBundle && !useWebpack) {
    web.wrapStatic(app, '/app.bundle./{ext}', './dist/app.bundle.');
    // web.wrapRouteGet(app, '/app.bundle.js', '/app.bundle.js', [],
    //     function(request) {
    //         var path = './dist/app.bundle.js';
    //         console.log('#/app.bundle.js', path);
    //         return wrapContentOrError(path);
    //     }, errorResponse
    // );
    ...
    //         console.log('#/app.bundle.css', path);
    //         return wrapContentOrError(path);
    //     }, errorResponse
    // );
}
...
function diseaseLandingHandler(request) {
    ...
    console.log('#pup_tent.render6', 'disease_main.mustache');
...
function phenotypeLandingHandler(request) {
    ...
    console.log('#pup_tent.render7', 'phenotype_main.mustache');
...
function modelByIdHandler(request, id, fmt) {
    try {
        ...
        console.log('#pup_tent.render8', 'node.mustache');
...
function genotypeByIdHandler(request, id, fmt) {
    try {
        ...
        console.log('#pup_tent.render9', 'node.mustache');
        var output = pup_tent.render('node.mustache', info, 'monarch_base.mustache');
        return web.wrapHTML(output);
    }
    catch(err) {
...
function caseByIdHandler(request, id, fmt) {
    try {
        ...
        console.log('#pup_tent.render10', 'node.mustache');
        var output = pup_tent.render('node.mustache', info, 'monarch_base.mustache');
        return web.wrapHTML(output);
    }
    catch(err) {
...
function geneLandingHandler(request) {
    ...
    console.log('#pup_tent.render11', 'gene_main.mustache');
...
function genotypeLandingHandler(request){
    ...
    console.log('#pup_tent.render12', 'genotype_main.mustache');
...
function modelLandingHandler(request){
    ...
    console.log('#pup_tent.render13', 'model_main.mustache');
...
function literatureLandingHandler(request){
    ...
    console.log('#pup_tent.render14', 'literature_main.mustache');
...
function literatureByIdHandler(request, id, fmt) {
    ...
    console.log('#pup_tent.render15', 'literature.mustache');
...
var scoreDocInfo = {
    description: 'Performs an annotation sufficiency calculation',
    paramInfo: {
        'categories': {
            description: 'Array of category ids'
        },
        'style': {
            description: 'E.g., phenotips'
        },
        'annotation_profile': {
            description: 'Object of the form {features: [entries]}, where each entry is...'
        }
    }
};

web.wrapGet(app, '/score', scoreHandler, errorResponse, scoreDocInfo);
web.wrapPost(app, '/score', scoreHandler, errorResponse, scoreDocInfo);
// Also allow trailing slashes
web.wrapGet(app, '/score/', scoreHandler, errorResponse, scoreDocInfo);
web.wrapPost(app, '/score/', scoreHandler, errorResponse, scoreDocInfo);

function feedbackHandler(request) {
    console.log('feedbackHandler', request);
...
web.wrapPost(app, '/feedback', feedbackHandler, errorResponse,
    {
        description: 'Submits a feedback entry from the Monarch feedback form',
        paramInfo: {
            'feedback-form-metadata': {
                description: 'Timestamps and browser client info'
            },
...
function annotateTextHandler(request) {
    ...
    console.log('#pup_tent.render16', 'annotate.mustache');
...
function annotateMinimalTextHandler(request, fmt) {
    ...
    console.log('#pup_tent.render17', 'annotate_minimal.mustache');
...
web.wrapRouteGet(app, '/labs', '/labs', [],
    function(request) {
        ...
        console.log('#pup_tent.render18', 'labs.mustache');
...
function homeHandler(request) {
  ...
  console.log('#pup_tent.render19', 'home-page.mustache');
...
function phenotypeByIdHandler(request, id, fmt){
    ...
    console.log('#pup_tent.render20', 'node.mustache');
...
function diseaseByIdHandler(request, id, fmt) {
    ...
    info.node_logo = '/image/carousel-diseases.png';
    console.log('#pup_tent.render21', 'node.mustache');
...
// Order matters here in the declaration of these routes.
web.wrapGet(app, '/disease/{id}.{fmt}', statusHandler, errorResponse,
    {
        description: 'Returns information about a specified Disease',
        paramInfo: {
            id: {
                    description: 'The Disease ID (e.g., "OMIM:305400")'
                }
        }
    });
...
function geneByIdHandler(request, id, fmt) {
    try {
        ...
        interaction_or_filter = [
                                 { field: 'subject', value: id },
                                 { field: 'object', value: id }
        ];
        addGolrTable(info, 'subject', 'interaction-table',
                interaction_filter, 'gene_interactions','#interactions', false, interaction_or_filter);

        info.interactionNum = engine.fetchAssociationCount(interaction_filter, null, interaction_or_filter);
        ...
        info.node_logo = '/image/carousel-genes.png';
        console.log('#pup_tent.render22', 'node.mustache');
        var output = pup_tent.render('node.mustache', info,
                 'monarch_base.mustache');
        return web.wrapHTML(output);
...
function variantByIdHandler(request, id, fmt) {
    try {
        ...
        info.node_logo = '/image/logo.png';
        console.log('#pup_tent.render23', 'node.mustache');
        var output = pup_tent.render('node.mustache', info,
                 'monarch_base.mustache');
        return web.wrapHTML(output);
...
function associationByIdHandler(request, id, fmt){
        ...
        try {
          ...
          //Curify ID if needed
            ...
            console.log('#pup_tent.render23', 'associations.mustache');
...
function analyzeByDatatypePostHandler(request, datatype, fmt) {
    ...
    console.log('#pup_tent.render23', 'analyze.mustache');
...
function analyzeByDatatypeGetHandler(request, datatype, fmt) {
    ...
    console.log('#pup_tent.render24', 'analyze.mustache');
     var output = pup_tent.render('analyze.mustache',info,'monarch_base.mustache');
     return web.wrapHTML(output);
}
...
web.wrapStatic(app, '/favicon.ico', './image/favicon.ico');
...
function rootPathHandler(request, id, fmt) {
    ...
    if (!id) {
        console.log('rootPathHandler', request.path);
        result = notFoundResponse('Unable to resolve path: ' + request.path);
    }
    else if (id.indexOf('apple-touch-icon') === 0) {
