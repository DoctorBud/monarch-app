...
function click(d) {
    if(d.nodeType > 1){
         ...
         *
        var options = document.getElementById("relationDropdown");
        options.style.marginTop = d.x + 20 + 'px';
        options.style.marginLeft = d.y + 120 + 'px';
        ...
        for (i = 0; i < dropdowns.length; i++) {
            ...
            var openDropdownKey = openDropdown.text;

            store[openDropdownKey] = i;

            (function(openDropdown){
                openDropdown.onclick = function(){
                    var type = store[openDropdown.text];

                    var dropBox = document.getElementsByClassName("dropdown-content");
                    for(var j = 0; j < dropBox.length; j++){
                        if (dropBox[j].classList.contains('show')) {
                            dropBox[j].classList.remove('show');
                        }
                    }
